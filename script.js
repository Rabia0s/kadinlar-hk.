...
document.addEventListener('keydown', function(event) {
    if (event.key === 'ArrowRight') {  // Sağ ok tuşuna basıldığında
        nextSlide();
    } else if (event.key === 'ArrowLeft') {  // Sol ok tuşuna basıldığında
        prevSlide();
    } });
...
videoCards.forEach(card => {
    card.addEventListener('click', () => {
        ...
        const videoUrl = card.querySelector('a') ? card.querySelector('a').href : card.getAttribute('data-video');
